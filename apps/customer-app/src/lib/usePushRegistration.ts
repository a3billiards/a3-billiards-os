/**
 * Registers native FCM token for push (admin broadcasts, booking alerts).
 * Retries when the user session becomes available.
 */
...
async function resolvePushToken(): Promise<string | null> {
  ...
  try {
    ...
    // Dev build missing google-services.json or Firebase not linked.
  }
